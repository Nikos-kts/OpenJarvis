...
class OrchestratorAgent(ToolUsingAgent):
    ...
    def _run_function_calling(
        ...
        **kwargs: Any,
        ...
        # Build initial messages — forward the configured system prompt so
        # callers like JarvisAgent (which pass a persona prompt via the
        # constructor) actually have it injected. Without this, function-
        # calling mode silently dropped the persona and the model fell back
        # to its own identity (e.g. "I'm Qwen…").
