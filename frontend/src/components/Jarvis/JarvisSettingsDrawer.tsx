...
type TabKey = 'persona' | 'intent' | 'voice' | 'wake' | 'tools' | 'delegation' | 'advanced';
...
export function JarvisSettingsDrawer({ agent, onClose, onUpdated }: Props) {
  const [draft, setDraft] = useState<JarvisPrimaryConfig>(agent.config || {});
  const [active, setActive] = useState<TabKey>('persona');
