                ...
                <main className="flex-1 flex flex-col items-center justify-center relative overflow-y-auto px-4 py-6">
                    ...
                    <div className="relative grid grid-cols-2 md:grid-cols-4 gap-3 mt-8 w-full max-w-3xl">
                        <HudTile
                            ...
                            value={
                                voiceEnabled
                                    ? `${cfg.voice?.tts_voice || 'Kore'}${cfg.voice?.realtime ? ' · live' : ''
                                    }`
                                    : 'Muted'
                            }
                            active={voiceEnabled}
...
function ChatBubble({ entry }: { entry: ChatEntry }) {
    ...
    return (
        <div
            className={`rounded-lg px-3 py-2 text-sm whitespace-pre-wrap ${isUser ? 'self-end max-w-[70%]' : 'self-start max-w-[80%]'
                }`}
            style={
                isUser
                    ? {
